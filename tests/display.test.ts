...
test("contributors are named in both languages, and unknown inputs by their key", () => {
  assert.equal(contributorLabel("usual_daily_change", "en"), "Usual change at this time of day");
  assert.equal(contributorLabel("usual_daily_change", "sw"), "Mabadiliko ya kawaida ya wakati huu wa siku");
...
const chirps: ChirpsContext = {
  ...
  chirps_dry_spell_days: 0, chirps_wet_spell_days: 10,
  valid_date: "2026-09-22", window_7d_start: "2026-09-16", window_30d_start: "2026-08-24",
  through: "2026-09-22T08:00:00.000Z",
};

test("rainfall totals are dated by the days they cover, both ending today", () => {
  const w = rainWindows(chirps);
  assert.ok(w);
  assert.equal(w.day, "2026-09-22");
  assert.deepEqual(w.week, { from: "2026-09-16", to: "2026-09-22" });
  assert.deepEqual(w.month, { from: "2026-08-24", to: "2026-09-22" });
});

test("unavailable rainfall has no windows to date", () => {
  const empty = Object.fromEntries(Object.keys(chirps).map((k) => [k, k === "available" ? false : null]));
  assert.equal(rainWindows(empty as ChirpsContext), null);
});
