...
import type { Lang, SituationResult } from "../src/lib/afya/types";
...
test("unavailable regional context is left out of the facts, not passed on as placeholders", () => {
  ...
  const unavailable = liveSituation({
    era5: { ...s.era5, available: false },
    chirps: { ...s.chirps, available: false },
  });
...
test("farm facts prefer the depth range to the single depth", () => {
  ...
  assert.match(String(farm.flat.farm_irrigation_reasons), /Root-zone water is substantially depleted/);
...
test("each intent answers its own question", () => {
  ...
  assert.match(template("en", "reliability"), /^Data quality is GOOD\. The latest station reading is 31 minutes old\..*0\.79°C/);
  ...
  assert.match(template("en", "heat"), /ventilation is relatively weak/);
