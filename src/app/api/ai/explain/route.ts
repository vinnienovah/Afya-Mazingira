...
async function farmFacts(situation: SituationResult, crop: string, stage: GrowthStage, lang: Lang): Promise<FarmFacts> {
  // Water advice needs real rainfall and soil moisture; without them it is
  // left out rather than built on placeholders, as on the Farm page.
  const waterAvailable = situation.era5.available !== false && situation.chirps.available !== false;
  try {
    return buildFarmExplanationFacts(await buildFarmAdvisory(situation, crop, stage), { waterAvailable, lang });
  } catch (err) {
