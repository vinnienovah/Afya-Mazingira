...
function templateEn({ facts: f, farm, intent, mode }: TemplateInput): string[] {
  ...
  const transition = () => {
    ...
    return `When this state has ended before, it most often gave way to ${f.transition_next_en} (${Math.round(f.transition_probability * 100)}% of past changes).`;
  };
...
function templateSw({ facts: f, farm, intent, mode }: TemplateInput): string[] {
  ...
  const transition = () => {
    ...
    return `Hali hii ilipoisha hapo awali, mara nyingi ilifuatiwa na ${f.transition_next_sw} (${Math.round(f.transition_probability * 100)}% ya mabadiliko yaliyopita).`;
  };
