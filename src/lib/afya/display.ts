...
/** The days behind the rainfall totals. Without explicit windows both end on valid_date. */
export function rainWindows(c: ChirpsContext): {
  day: string;
  week: { from: string; to: string };
  month: { from: string; to: string };
} {
  const day = c.valid_date;
  return {
    day,
    week: c.window_7d ?? { from: addDays(day, -6), to: day },
    month: c.window_30d ?? { from: addDays(day, -29), to: day },
  };
}
